...
namespace MultiShop.Catalog.Services.CategoryServices
{
	public class CategoryService : ICategoryService
	{
		...
		public CategoryService(IMapper mapper, IConfiguration configuration)
		{
			...
			var categoryCollectionName = configuration.GetSection("DatabaseSettings:CategoryCollectionName").Value;

			var client = new MongoClient(connectionString);
			var database = client.GetDatabase(databaseName);

			_categoryCollection = database.GetCollection<Category>(categoryCollectionName);
			_mapper = mapper;
		}
